...
private:
	union {
		const char* _texName; ///< Pathname of the texture map.
		GLuint _texId;   ///< Texture ID
	};
	bool useTexId;

	const char* _normalMapName; ///< Pathname of the normal map.
	...
	const char* _modelName; ///< Pathname of the model. Used to look up render object.
...
public:
	...
	*/
	inline DrawableEntity(const char* tn, const char* mn, WorldEntity* parent=NULL)
